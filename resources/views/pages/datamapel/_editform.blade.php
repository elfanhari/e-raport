    ...
    <div class="form-group row">
      ...
      <div class="col-sm-8">
        <select class="form-select @error('kelompok') is-invalid @enderror" name="kelompok" id="">
          ...
          <option value="A" {{ 'A' == old('kelompok', $mapel->kelompok) ? 'selected' : '' }}>A</option>
          <option value="B" {{ 'B' == old('kelompok', $mapel->kelompok) ? 'selected' : '' }}>B</option>
        </select>
        @error('tingkat')
        <span class="invalid-feedback mt-1">
          {{ $message }}
      ...
      <div class="col-sm-8">
        <select class="form-select @error('tapel_id') is-invalid @enderror" name="tapel_id" id="exampleSelectBorder">
          ...
          @foreach ($tapel as $item)
            <option value="{{ $item->id }}" {{ $item->id == old('tapel_id', $mapel->tapel_id) ? 'selected' : ''}}>{{ $item->tahun_pelajaran }} - {{ $item->semester == 1 ? 'Ganjil' : 'Genap' }}</option>
          @endforeach
        </select>
        @error('tapel_id')
        <span class="invalid-feedback mt-1">
